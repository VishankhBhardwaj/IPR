...
const getRowKey = (table) => {
  ...
  if (table.rows?.some((row) => row.country !== undefined) || table.grandTotal?.country !== undefined) {
    return 'country';
  }
  return 'year';
};
        ...
        <table className="analysis-table">
          ...
          <tbody>
            {rows.map((row) => {
              ...
              return (
                <tr key={`${table.title}-${row[rowKey]}`} className={isGrandTotal ? 'grand-total-row' : ''}>
                  <td>{row[rowKey]}</td>
                  {(table.columns || []).map((column) => (
...
const Analysis = () => {
  ...
  const fetchAnalysis = useCallback(async (nextFilters = emptyFilters) => {
    try {
      ...
      setAnalysis(null);
      setError(err.response?.data?.message || 'Failed to load analysis. Please check login and backend server.');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  return (
    <div className="analysis-page container animate-fade-in" >
      <div className="analysis-header" style={{marginTop: '50px'}}>
        <div>
          <h1>Analysis</h1>
          <p className="subtitle">Year-wise, type-wise, and country-wise patent summaries.</p>
        </div>
        <BarChart3 size={34} />
      </div>
{/* 
        ...
        <>
          <div className="analysis-metrics">
            {metricCards.map((metric) => (
              <section className="analysis-metric clean-panel" key={metric.label}>
